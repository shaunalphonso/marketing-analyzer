...
[📧 Contact](mailto:shaun@georgian.io) | [🌐 LinkedIn](https://www.linkedin.com/in/shaunalphonso/)
    ...
    with col_center:
        ...
        # Progress tracker appears right after the button is clicked
        ...
        # Example buttons (centered under features)
    ...
    if 'example_url' in st.session_state:
        url = st.session_state.pop('example_url', None)
        analyze_button = True

    # Main analysis flow — show progress & results in center column
    if analyze_button and url:
        if not url.startswith(('http://', 'https://')):
            url = 'https://' + url

        # Keep analysis results in center column
        with col_center:

        # Keep analysis results in center column
        with col_center:
            # Get the progress elements that were created above
            if 'progress_container' in locals() and 'progress_bar' in locals() and 'status_text' in locals():
                try:
                    # Step 1: Scrape
                    ...
                    for key, value in analysis_results.items():
                        if key not in ["URL", "Content_Length", "error"] and not str(value).startswith("Analysis error"):
                            st.markdown(f"**{key}:**")
                            st.info(value)
